...
show_logs() {
    ...
    for line in "${log_lines[@]}"; do
        address=$(echo "$line" | grep -oP '^\S+')  # Extract the IP address
        date=$(echo "$line" | grep -oP '\[\K[^]]+')  # Extract the date and time
        method=$(echo "$line" | grep -oP '\"[A-Z]+(?= )')  # Extract the HTTP method
        url=$(echo "$line" | grep -oP '\"[A-Z]+ \K\S+(?= )')  # Extract the URL
